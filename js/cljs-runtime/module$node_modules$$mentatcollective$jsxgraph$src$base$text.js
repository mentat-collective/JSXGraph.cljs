...
h.visPropCalc.visible&&"axis"!==h.elType&&"ticks"!==h.elType&&h!==this.board.infobox&&h!==this&&h.hasPoint(x,y)&&count++;this.board.options.precision.hasPoint=savePointPrecision;return count},setAutoPosition:function(){var w=this.size[0],h=this.size[1],JSCompiler_object_inline_r_3143,step=2*Math.PI/12;if(this===this.board.infobox||!this.visPropCalc.visible||!_type.default.evaluate(this.visProp.islabel)||!this.element)return this;var delta_r=_type.default.evaluate(this.visProp.offset);var cy=this.element.getLabelAnchor();
var cx=cy.scrCoords[1];cy=cy.scrCoords[2];var start_angle=delta_r[0];var angle=delta_r[1];var x=this.getNumberofConflicts(cx+start_angle,cy-angle,w,h);if(0===x)return this;var r=12;delta_r=.2*r;start_angle=Math.atan2(angle,start_angle);var JSCompiler_object_inline_conflicts_3141=x;var JSCompiler_object_inline_angle_3142=start_angle;for(JSCompiler_object_inline_r_3143=r;0<JSCompiler_object_inline_conflicts_3141&&28>r;){var j=1;for(angle=start_angle+step;12>j&&0<JSCompiler_object_inline_conflicts_3141;j++){x=
Math.cos(angle);var y=Math.sin(angle);x=cx+r*x;y=cy-r*y;x=this.getNumberofConflicts(x,y,w,h);x<JSCompiler_object_inline_conflicts_3141&&(JSCompiler_object_inline_conflicts_3141=x,JSCompiler_object_inline_angle_3142=angle,JSCompiler_object_inline_r_3143=r);if(0===JSCompiler_object_inline_conflicts_3141)break;angle+=step}r+=delta_r}r=JSCompiler_object_inline_r_3143;x=Math.cos(JSCompiler_object_inline_angle_3142);y=Math.sin(JSCompiler_object_inline_angle_3142);this.visProp.offset=[r*x,r*y];this.visProp.anchorx=
...
"_form",board.rendNodeRange.id=board.rendNode.id+"_range",board.rendNodeOut.id=board.rendNode.id+"_out"}catch(e){_jxg.default.debug(e)}board.rendNodeRange.style.width=attributes.widthrange+"px";board.rendNodeRange.style.verticalAlign="middle";board.rendNodeOut.style.width=attributes.widthout+"px";board._val=parents[1][1];_jxg.default.supportsVML()?_env.default.addEvent(board.rendNodeForm,"change",JSCompiler_object_inline_HTMLSliderInputEventHandler_3144,board):_env.default.addEvent(board.rendNodeForm,
"input",JSCompiler_object_inline_HTMLSliderInputEventHandler_3144,board);board.Value=function(){return this._val};return board};_jxg.default.registerElement("htmlslider",_jxg.default.createHTMLSlider);exports.default={Text:_jxg.default.Text,createText:_jxg.default.createText,createHTMLSlider:_jxg.default.createHTMLSlider}}
